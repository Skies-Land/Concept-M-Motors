// TESTING LIBRARY - utils de rendu et sélection DOM
import { render, screen } from '@testing-library/react';

// VITEST - framework de test
import { describe, it, expect } from 'vitest';

// COMPOSANT À TESTER
import BookingAccount from './4-Booking-account';

describe("BookingAccount - Composant de gestion des réservations client", () => {
    
    it("doit afficher le titre et la description de la section", () => {
        // 1. ARRANGE (préparation - initialisation du composant)
        render(<BookingAccount />);

        // 2. ASSERT (vérification - affichage du titre et de la description)
        expect(screen.getByText(/Mes réservations/i)).toBeInTheDocument();
        expect(screen.getByText(/Planifiez et gérez vos rendez-vous en concession/i)).toBeInTheDocument();
    });

    it("doit afficher le message d'attente (bientôt disponible)", () => {
        // 1. ARRANGE (préparation - initialisation du composant)
        render(<BookingAccount />);

        // 2. ASSERT (vérification - affichage du message d'attente)
        expect(screen.getByText(/Votre historique de réservations apparaîtra bientôt ici.../i)).toBeInTheDocument();
    });
});